...
function [info] = adxs(info,high_prices,low_prices,p)    
%% Load Params
adx_period = 14;
adx_thres = 25;

%% Preallocation Vectors
dmup = zeros(size(high_prices,1),1);
dmdn = zeros(size(high_prices,1),1);
smtr = zeros(size(high_prices,1),1);
smdmup = zeros(size(high_prices,1),1);
smdmdn = zeros(size(high_prices,1),1);
zros = zeros(size(high_prices,1),1);
zerohigh = zeros(size(high_prices,1),1);
zerolow = zeros(size(high_prices,1),1);
highdiff = zeros(size(high_prices,1),1);
lowdiff = zeros(size(high_prices,1),1);
diup = zeros(size(high_prices,1),1);
didn = zeros(size(high_prices,1),1);
didiff = zeros(size(high_prices,1),1);
disum = zeros(size(high_prices,1),1);
dx = zeros(size(high_prices,1),1);
adx = zeros(size(high_prices,1),1);
adx_dihistogram = zeros(size(high_prices,1),1);
adx_disignalcross = zeros(size(high_prices,1),1);
adx_levelcross = zeros(size(high_prices,1),1);
adx_ditrend = zeros(size(high_prices,1),1);  


%% Create TR Variable from info structure
tr = info.strat_indicators.atr.tr;
...
highdiff(2:end,1) = high_prices(2:end,1)-high_prices(1:end-1,1);
lowdiff(2:end,1) = low_prices(1:end-1,1)-low_prices(2:end,1);
...
abholder = zeros(size(high_prices,1),1);
crossholder = zeros(size(high_prices,1),1);
holder = zeros(size(high_prices,1),1);
...
%% ADX Structure Build
info.ammo.stage(p).strat_indicators.adx.adx = adx;
info.ammo.stage(p).strat_indicators.adx.adx_dihistogram = adx_dihistogram;
info.ammo.stage(p).strat_indicators.adx.adx_disignalcross = adx_disignalcross;
info.ammo.stage(p).strat_indicators.adx.adx_levelcross = adx_levelcross;
info.ammo.stage(p).strat_indicators.adx.adx_ditrend = adx_ditrend;
