...
function [info] = stochastic(info,close_prices,high_prices,low_prices,p)
try
%% Build Params
so_fk_period = info.params.strat_params.so_fk_period;
so_sk_period = info.params.strat_params.so_sk_period;
so_sd_period = info.params.strat_params.so_sd_period;
so_above_thres = info.params.strat_params.so_above_thres;
so_below_thres = info.params.strat_params.so_below_thres;
...
lowestlow = zeros(size(close_prices,1),1);
highesthigh = zeros(size(close_prices,1),1);
so_fastpctk = zeros(size(close_prices,1),1);
             
%% K/D calculations fast,slow
i=0;
for i =  1:size(close_prices,1)
    try
        lowestlow(i,1) = min(low_prices(i:i+so_fk_period_lag,1));
        highesthigh(i,1) = max(high_prices(i:i+so_fk_period_lag,1));
        so_fastpctk(i,1) = ((close_prices(i,1)-lowestlow(i,1))/(highesthigh(i,1)-lowestlow(i,1)))*100;
    catch
        continue
    end
end
...
info.ammo.stage(p).strat_indicators.stochastic.so_fastpctd = so_fastpctd;
info.ammo.stage(p).strat_indicators.stochastic.so_slowpctd = so_slowpctd;
info.ammo.stage(p).strat_indicators.stochastic.so_histogram = so_histogram;

catch ME
    sendmail('ammodono@gmail.com', 'ERROR: stochastic',['identifier: ' ME.identifier 10 'message: ' ME.message 10 'function path: ' ME.stack.file 10 'function name: ' ME.stack.name 10 'line: ' num2str(ME.stack.line)]);
end
end
